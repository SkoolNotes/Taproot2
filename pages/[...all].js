...
const PageRenderer = () => {
    ...
    if (router.query.all !== undefined) {
        ...
        return (<>
            <span className="sticky block shadow-lg font-bold p-4 text-red-500 text-xl top-0 bg-gray-100 dark:bg-gray-dark z-40">Taproot <span style={{float: "right", fontWeight: 500}}>{theFile.data.title}</span></span>
            <div className="content">
                <div className="datarow">
                    <span className="datarow-author">{theFile.data.author}</span> | <span className="datarow-course">{theFile.data.course}<span className="datarow-back"><a href={`./${theFile.data.source}`} className="text-red-500 hover:text-red-800 transition" style={{display: theFile.data.source?"inline":"none"}}>[[{theFile.data.source}]]</a><FontAwesomeIcon icon={faCaretRight} style={{transform: "translateY(2px)", margin: "0 6px", display: theFile.data.source?"inline":"none"}}/><a href={`./${note}`} className="text-red-500 hover:text-red-800 transition">[[{note}]]</a></span></span>
                </div>
